...
*Response:* The two covariates that look most different between MPA and non-MPA sites are:
- pollution_risk(low)
...
*Response:* reef_exposure (exposed). Exposed reefs may receive more nutrients and water flow, naturally boosting fish biomass. At the same time, exposed reefs might be less likely to be chosen as MPAs because they're harder to protect. This creates confounding. We might think MPAs have less effect when really it's just that non-MPA sites happen to have better natural conditions.
...
*Response:* `ATT` in this context means the the actual weight of the fish compared to their counterfactual weights, what would have happened without treatment.
...
```
**The biomass in the fisheries increased by 107kg/ha.**
...
*Response:* On average,treatment sites have 107.78 kg/ha higher fisheries biomass compared to control sites.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* We have unobserved confounding variables that we did not observe previously.
...
*Response:* `vpd` in the `no-FE model` is 0.126, and `vpd` in the `FE model` is 0.227. The estimate got larger. Some bias from plot by plot scenario was confounding the unobserved plot differences.
...
*Response:* `year` is included as fixed effects because `year` controls time varying variables. One example of “year shock” that could bias estimates if not controlled is wet and dry years. The rainfall is high in wet year, and the rainfall is low in dry year.
...
*Response:* `plot` is included as fixed effects because `plot` stay constant over time and it controls the characteristics of each plot. One example of a “time-invariant plot difference” is soil quality in each plot. The soil quality is permanent differences between plots, and it could bias estimates because we might attribute the differences in outcomes to main variable.
