...
**RESPONSE:** There are several reasons why PM2.5 and the error term in the asthma equation could be correlated.

*Omitted variable bias:* Many factors simultaneously affect both PM2.5 levels and respiratory illness searches. For example, seasonal patterns (cold/flu season increases respiratory searches and also coincides with weather inversions that trap pollution), wildfire season (increases PM2.5 and also generates smoke-related search behavior independently), economic activity levels, or urbanization/traffic patterns.

*Reverse causality:* While less obvious here, one could argue that areas where people experience more respiratory illness may attract more monitoring or policy attention, subtly affecting measured pollution levels.

*Measurement error:* PM2.5 is measured at monitoring stations and averaged across the county, which may not reflect actual individual exposure. Google Trends is a noisy proxy for actual respiratory illness.
...
**RESPONSE:** Yes. Even after absorbing seasonal patterns, precipitation could directly affect asthma searches through mechanisms other than PM2.5:

- Cold, damp air during a rainstorm can trigger bronchospasm in asthma sufferers, directly increasing inhaler searches.
- Mold growth increases after rainy periods, which is an independent asthma trigger unrelated to PM2.5.
- Behavioral changes: Rain may keep people indoors, changing their exposure to indoor allergens (dust, pets) and also changing their likelihood of searching online.
- Thunderstorm asthma: Certain storm events can break up pollen grains into respirable particles, triggering asthma attacks directly.
...
```{r}
pm25 <- read_csv(here::here("week6", "data", "daily_pm25_LA.csv")) %>% 
  janitor::clean_names() %>%  
...
```{r}
options(noaakey = "pvyswHGpjDDtGlFkFGytNyJtHuushzTb")
```
...
**RESPONSE:** We have 53 observations, weekly data for one year. This is quite small for regression analysis.

Implications include: low statistical power (hard to detect true effects), imprecise estimates (wide confidence intervals), sensitivity to outliers, and difficulty supporting many control variables (the month fixed effects alone use 11 degrees of freedom, leaving only 40).
...
**RESPONSE:** The coefficient went from 0.205 (naive OLS) to 0.588 (OLS + month FE). It increased in magnitude, suggesting that seasonal confounders were actually masking part of the positive association. This could happen if, for example, summer months have higher PM2.5 but lower asthma searches (people are outdoors, fewer respiratory infections), creating a negative confounding bias that suppresses the naive estimate. However, neither estimate is statistically significant.

Even with month FE, the OLS estimate may still be biased because there are non-seasonal omitted variables (e.g., wildfire events, traffic shocks, local construction activity) that affect both PM2.5 and respiratory illness but aren't captured by monthly dummies.
...
**RESPONSE:** The coefficient on precip_total is negative (-0.100), which is consistent with expectations — more rain washes particulates out of the air, reducing PM2.5 (wet deposition).

However, the coefficient is not statistically significant, and the F-statistic is about 3.16, well below the rule-of-thumb threshold of 10. This means precipitation is a weak instrument, which is a serious concern: weak instruments can make IV estimates biased toward OLS and produce unreliable inference with inflated standard errors.
...
**RESPONSE:** As discussed above: cold damp air as a direct trigger, mold growth, thunderstorm asthma, behavioral changes (staying indoors). If precipitation directly reduces asthma symptoms (e.g., by reducing pollen or dust), the IV estimate would be biased away from zero (overstating the effect). If precipitation directly increases asthma symptoms through a non-PM2.5 channel, the bias direction depends on the correlation structure.
...
**RESPONSE:** Plausible improvements include: using data from a longer time period (multiple years) to increase sample size and variation; using more granular geographic variation (multiple monitoring stations across locations); focusing on specific storm events rather than total weekly precipitation; or using a different climate region where precipitation varies more and has a stronger relationship with PM2.5.
...
**RESPONSE:** A good alternative instrument could be wind speed/direction:
Relevance: Strong winds disperse particulate matter, reducing PM2.5. Wind direction determines whether pollution from industrial sources or highways is blown toward monitors.

Exclusion restriction: Wind patterns plausibly affect respiratory illness only through their effect on air quality, not directly (especially after controlling for season and temperature).

Another option: thermal inversions (temperature inversions that trap pollution near the surface). These are driven by atmospheric conditions, strongly predict PM2.5 spikes, and arguably don't affect respiratory health except through pollution.
...
**RESPONSE:** Relevance (First Stage): The instrument Z (precipitation) must be correlated with the endogenous variable X (PM2.5). Here: rain must actually predict PM2.5 levels. (Our weak F-stat raises concerns.)

Exclusion Restriction: Z must affect Y (asthma searches) only through X (PM2.5), with no direct or backdoor effect. Here: rain must not independently trigger asthma searches through non-air-quality channels.

Independence (Exogeneity of the instrument): Z must be uncorrelated with the error term in the outcome equation — i.e., uncorrelated with unobserved determinants of Y. Here: precipitation should not be correlated with other omitted factors driving asthma searches (partially addressed by month FE).

Monotonicity (for LATE interpretation): The instrument must affect the endogenous variable in the same direction for all observations. Here: more rain should never increase PM2.5 — it should always reduce it (or have no effect). This is generally plausible for wet deposition.
